...
const breadcrumbs = (engineer: EngineerResource): BreadcrumbItem[] => [
    ...
    {
        title: engineer.name,
        href: engineersRoutes.edit.url({ engineer: engineer.id }),
    },
];
...
export default function EngineerEdit({ engineer, provinces }: Props) {
    const { data, setData, put, processing, errors } = useForm<FormData>({
        ...
        experience_years: engineer.experience_years
            ? String(engineer.experience_years)
            : '',
    ...
    const handleDelete = () => {
        if (confirm(`Remove ${engineer.name} from the directory?`)) {
            router.delete(
                engineersRoutes.destroy.url({ engineer: engineer.id }),
            );
        }
    };

    const provinceValue = data.primary_province_id
        ? String(data.primary_province_id)
        : '';
        ...
        <AppLayout breadcrumbs={breadcrumbs(engineer)}>
            ...
            <div className="space-y-6 pb-10">
                <section className="relative overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-6 shadow-lg starting:translate-y-4 starting:opacity-0">
                    <div className="bg-[radial-gradient(circle_at_top,_theme(colors.primary/12),_transparent_65%)] absolute inset-0 -z-10" />
                    <div className="flex flex-wrap items-start justify-between gap-6">
                        <div className="space-y-4">
                            <Badge
                                variant="outline"
                                className="w-fit rounded-full border-white/30 bg-white/20 text-xs font-semibold tracking-[0.35em] text-white uppercase dark:text-foreground"
                            >
                                Engineer Record
                            </Badge>
                            ...
                            <p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
                                Adjust core details, update coverage, and review
                                active Special Place assignments for this
                                engineer.
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <Link href={engineersRoutes.index.url()}>
                                <Button
                                    variant="ghost"
                                    className="rounded-full border border-white/40 bg-white/20 backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/40"
                                >
                        ...
                        {[
                            {
                                title: 'Profile',
                                description:
                                    'Refresh identification and contact information.',
                                icon: ClipboardEdit,
                            },
                            {
                                title: 'Coverage',
                                description:
                                    'Keep province alignment current for dispatch.',
                                icon: Sparkles,
                            },
                            {
                                title: 'Assignments',
                    ...
                    <div className="space-y-6">
                        <Card className="border border-border/60">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Keep engineer information fresh to ensure
                                    accurate scheduling.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="grid gap-6 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="primary_province_id">
                                        Primary province
                                    </Label>
                                    <Select
                                        value={provinceValue}
                                    ...
                                    <InputError
                                        message={errors.primary_province_id}
                                    />
                                </div>
                                ...
                                <div className="space-y-2 md:col-span-2">
                                    ...
                                    <Textarea
                                        ...
                                        placeholder="Update any certifications, preferred shifts, or latest onboarding notes."
                                    />
                        ...
                        <Card className="border border-border/60">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Control visibility across Special Place
                                    workflows.
                                </CardDescription>
                            </CardHeader>
                                ...
                                <div className="flex items-center justify-between rounded-2xl border border-dashed border-primary/40 bg-primary/5 p-4">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Inactive engineers will be hidden
                    ...
                    <aside className="space-y-6">
                        <Card className="sticky top-28 border border-border/60">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Current dedicated locations linked to this
                                    engineer.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4 text-sm">
                                {engineer.special_places.length === 0 && (
                                    <p className="text-muted-foreground">
                                        No Special Places assigned yet.
                                    </p>
                                )}
                                {engineer.special_places.map((place) => (
                                    ...
                                    >
                                        ...
                                        <Badge
                                            variant="outline"
                                            className={cn(
                                                'mt-2 inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold',
                                                place.is_active
                                                    ? 'border-emerald-500/40 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-200'
                                                    : 'border-slate-500/40 bg-slate-100 text-slate-700 dark:bg-slate-900/40 dark:text-slate-200',
                                            )}
                                        >
                                            {place.is_active
                                                ? 'Active'
