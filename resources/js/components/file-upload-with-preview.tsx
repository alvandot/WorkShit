...
import { Check, File, FileImage, Loader2, Upload, X } from 'lucide-react';
                ...
                canvas.toBlob(
                    (blob) => {
                        ...
                        try {
                            ...
                            webpBlob.name = file.name.replace(
                                /\.[^/.]+$/,
                                '.webp',
                            );
...
export default function FileUploadWithPreview({
    ...
    const handleFiles = async (newFiles: FileList | null) => {
        ...
        try {
            ...
            for (const file of filesArray) {
                ...
                if (disableWebpConversion) {
                    ...
                    try {
                        ...
                        alert(
                            `Could not convert ${file.name}. Please try another file.`,
                        );
                    }
                }
            }
            ...
            const newFilesWithPreview: FileWithPreview[] = validFiles.map(
                (file) => {
    ...
    return (
        <div className="space-y-3">
            ...
            >
                ...
                <div className="flex flex-col items-center gap-3">
                    ...
                    >
                        {isConverting ? (
                            ...
                            <Check className="size-8 animate-in text-green-500 duration-200 zoom-in-50" />
                        ) : (
                            <Upload
                                className={cn(
                ...
                {/* Success Overlay */}
                {showSuccess && (
                    <div className="absolute inset-0 flex animate-in items-center justify-center rounded-lg bg-green-500/10 duration-300 fade-in-0">
            ...
            {/* Error Message */}
            {error && (
                <p className="animate-in text-sm text-destructive duration-200 slide-in-from-top-1">
                    {error}
                </p>
            )}
                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                        {existingFiles.map((fileUrl, index) => (
                            <div
                                key={index}
                                className="group relative aspect-square overflow-hidden rounded-lg border bg-muted/50 transition-colors duration-200 hover:bg-muted"
                            >
                                {fileUrl.match(
                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                        {filesWithPreview.map((item) => (
                            <div
                                key={item.id}
                                className="group relative aspect-square animate-in overflow-hidden rounded-lg border bg-muted/50 duration-300 fade-in-0 zoom-in-95"
                            >
                                {/* Preview */}
                                {item.preview ? (
                                    <img
                                ...
                                <button
                                    ...
                                    className={cn(
                                        'absolute top-2 right-2 rounded-full bg-destructive p-1.5 text-destructive-foreground',
                                        'opacity-0 transition-all duration-200 group-hover:opacity-100',
                                        'hover:scale-110 hover:bg-destructive/90',
                                        'focus:opacity-100 focus:ring-2 focus:ring-destructive focus:ring-offset-2 focus:outline-none',
                                    )}
                                ...
                                {/* File Info Overlay */}
                                <div className="absolute right-0 bottom-0 left-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
